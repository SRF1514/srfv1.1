...
export default function MarketModulePage() {
  ...
  const currentIndex = marketFoundationsModules.findIndex(m => m.id === id);
  ...
  if (currentIndex === -1) {
    return <Navigate to="/learn/market-foundations" replace />;
  }

  const module = marketFoundationsModules[currentIndex];
  const prevModule = currentIndex > 0 ? marketFoundationsModules[currentIndex - 1] : null;
  const nextModule = currentIndex < marketFoundationsModules.length - 1 ? marketFoundationsModules[currentIndex + 1] : null;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-12 lg:gap-20 relative">
          ...
          <div className="w-full">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center pt-8 border-t border-gray-100 mt-16 gap-6">
              {prevModule ? (
                <Link 
                  to={`/learn/market-foundations/${prevModule.id}`}
              ...
              {nextModule ? (
                <Link 
                  to={`/learn/market-foundations/${nextModule.id}`}
          ...
          <ModuleSidebar 
            modules={marketFoundationsModules}
            currentId={id!}
            isSidebarOpen={isSidebarOpen}
            setIsSidebarOpen={setIsSidebarOpen}
            basePath="/learn/market-foundations"
          />
        </div>
      </div>
    </div>
