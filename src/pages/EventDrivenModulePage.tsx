...
export default function EventDrivenModulePage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  
  const currentIndex = eventDrivenAnalysisModules.findIndex(m => m.id === id);
  ...
  if (currentIndex === -1) {
    return <Navigate to="/learn/event-driven-analysis" replace />;
  }

  const module = eventDrivenAnalysisModules[currentIndex];
  const prevModule = currentIndex > 0 ? eventDrivenAnalysisModules[currentIndex - 1] : null;
  const nextModule = currentIndex < eventDrivenAnalysisModules.length - 1 ? eventDrivenAnalysisModules[currentIndex + 1] : null;
  ...
  return (
    <div className="min-h-screen bg-white text-[#1a1a1a] pt-[112px] lg:pt-[140px] font-sans">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-8 lg:px-[2cm] pb-24">
        <Link 
          to="/learn/event-driven-analysis" 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-12 lg:gap-20 relative">
          ...
          <div className="w-full">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center pt-8 border-t border-gray-100 mt-16 gap-6">
              {prevModule ? (
                <Link 
                  to={`/learn/event-driven-analysis/${prevModule.id}`}
              ...
              {nextModule ? (
                <Link 
                  to={`/learn/event-driven-analysis/${nextModule.id}`}
          ...
          <ModuleSidebar 
            modules={eventDrivenAnalysisModules}
            currentId={id!}
            isSidebarOpen={isSidebarOpen}
            setIsSidebarOpen={setIsSidebarOpen}
            basePath="/learn/event-driven-analysis"
          />
        </div>
      </div>
    </div>
