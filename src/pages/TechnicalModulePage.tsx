...
export default function TechnicalModulePage() {
  ...
  const currentIndex = technicalAnalysisModules.findIndex(m => m.id === id);
  ...
  if (currentIndex === -1) {
    return <Navigate to="/learn/technical-analysis" replace />;
  }

  const module = technicalAnalysisModules[currentIndex];
  const prevModule = currentIndex > 0 ? technicalAnalysisModules[currentIndex - 1] : null;
  const nextModule = currentIndex < technicalAnalysisModules.length - 1 ? technicalAnalysisModules[currentIndex + 1] : null;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-12 lg:gap-20 relative">
          ...
          <div className="w-full">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center pt-8 border-t border-gray-100 mt-16 gap-6">
              {prevModule ? (
                <Link 
                  to={`/learn/technical-analysis/${prevModule.id}`}
              ...
              {nextModule ? (
                <Link 
                  to={`/learn/technical-analysis/${nextModule.id}`}
          ...
          <ModuleSidebar 
            modules={technicalAnalysisModules}
            currentId={id!}
            isSidebarOpen={isSidebarOpen}
            setIsSidebarOpen={setIsSidebarOpen}
            basePath="/learn/technical-analysis"
          />
        </div>
      </div>
    </div>
